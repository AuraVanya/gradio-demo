...
def analyze_image_with_rekognition(s3_key):
    ...
    try:
        ...
        # Detect moderation labels (explicit content)
...
def process_image(file):
    ...
    for label in results.get('moderation_labels', []):
        table_data.append({
            'Category': 'Moderation',
            'Name': label['name'],
            'Confidence': f"{label['confidence']}%",
            'Details': label['parent_name']
        })
...
# Image Analyzer Gradio Interface
image_analyzer = gr.Interface(
    fn=process_image,
    inputs=gr.File(file_types=[".png", ".jpg", ".jpeg"], label="Upload Image"),
    outputs=[
...
demo = gr.TabbedInterface([acord_extractor, faq_chatbot, image_analyzer], ["ACORD Extractor", "FAQ Chatbot", "Image Analyzer"])
